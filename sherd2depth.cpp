...
extern "C"
int SplitCloud(char* in_file, char* out_dir, double min_height_percent, double min_size_percent)
{
	PointCloud::Ptr src_cloud(new PointCloud);
	bool if_read = ReadXYZ(in_file, src_cloud);
  if (!if_read)
    return -1;
  else
  {
    ...
    //write split clouds to xyz files
      ...
      char c_char = '1' + c;
      string out_file = in_file_str.substr(0, in_file_str.size()-4) + '-' + c_char + ".xyz";
      WriteXYZ(out_file, split_cloud_vec[c]);
		}
