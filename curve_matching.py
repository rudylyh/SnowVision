...
def GetTopKMatch(opts, matcher, sherd_name, depth_img, curve_img, mask_img, design_dir):
    resize_scale = opts['resize_scale']
    batch_size = opts['batch_size']
    patch_size = opts['patch_size']
    sherd_patch_stride = opts['sherd_patch_stride']
    design_patch_stride = opts['design_patch_stride']
    feat_len = opts['feat_len']
    top_k = opts['top_k']
    ...
    for j, design_name in enumerate(os.listdir(design_dir)):
        ...
        while k < design_patch_imgs.shape[0]:
            design_patch_feats = np.vstack((design_patch_feats, matcher.img2feat(design_patch_imgs[k:k + batch_size])))
            k += batch_size
        sherd2design_dist, sherd2design_idx = matcher.GetSherdToDesignPatchDists(sherd_patch_imgs, design_patch_feats)
    ...
    for k in range(0, top_k):
        ...
        [dx, dy] = matcher.design_patch_loc_dict[design_name][design_patch_idx]
        design_img = cv2.imread(os.path.join(design_dir, design_name + '.png'), 0)
        design_img = cv2.resize(design_img, None, fx=resize_scale, fy=resize_scale)
...
def WriteMatchResults(top_k_match, save_dir):
